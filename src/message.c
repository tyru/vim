...
screen_puts_mbyte(char_u *s, int l, int attr)
{
    ...
    screen_puts_len(s, l, msg_row, msg_col + vtabline_width(), attr);
#ifdef FEAT_RIGHTLEFT
    if (cmdmsg_rl)
    {
	msg_col -= cw;
    ...
    int		attr)
{
    /* output postponed text */
    msg_didout = TRUE;		/* remember that line is not empty */
    screen_puts_len(t_s, (int)(s - t_s), msg_row, msg_col + vtabline_width(), attr);
    msg_col += *t_col;
    *t_col = 0;
#ifdef FEAT_MBYTE
...
do_more_prompt(int typed_char)
{
	...
	if (toscroll != 0)
	{
		...
		while (toscroll > 0 && mp_last != NULL)
		{
		    ...
		    msg_scroll_up();
		    inc_msg_scrolled();
		    screen_fill((int)Rows - 2, (int)Rows - 1, 0 + vtabline_width(),
		...
		screen_fill((int)Rows - 1, (int)Rows, 0 + vtabline_width(),
						   (int)Columns + vtabline_width(), ' ', ' ', 0);
		msg_moremsg(FALSE);
		continue;
	    }
...
msg_clr_eos_force(void)
{
	...
	if (cmdmsg_rl)
	{
	    screen_fill(msg_row, msg_row + 1, 0 + vtabline_width(), msg_col + 1 + vtabline_width(), ' ', ' ', 0);
	    screen_fill(msg_row + 1, (int)Rows, 0 + vtabline_width(), (int)Columns + vtabline_width(), ' ', ' ', 0);
	}
	else
#endif
	{
	    screen_fill(msg_row, msg_row + 1, msg_col + vtabline_width(), (int)Columns + vtabline_width(),
								 ' ', ' ', 0);
	    screen_fill(msg_row + 1, (int)Rows, 0 + vtabline_width(), (int)Columns + vtabline_width(), ' ', ' ', 0);
	}
    }
